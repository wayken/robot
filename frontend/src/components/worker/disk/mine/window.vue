<template>
   <el-dialog draggable width="580px" append-to-body class="a-disk-window" :title="title"
    :model-value="visible"
...
const handleDataLoad = () => {
  ...
  ioRequest('disk.index', params).then((result: any) => {
    const list = result[0] || []
    // 只显示文件夹
    dataList.value = list.filter((data: FolderItem) => data.directory)
  })
}
// 单击选中文件夹
