  ...
  <div class="panel inline-flex-c-n-n">
    ...
    <template v-else>
      <a-nodata :loading="false" :success="true" size="large"
        :description="$t('rules.document-empty-description')"
      />
    </template>
  </div>
</template>
...
const handleEmit = defineEmits<{
  ...
  (e: 'remove', file: RuleFile): void
  (e: 'rename', file: RuleFile, newFilename: string): void
}>()
...
const handleDocumentRemove = (file: RuleFile) => {
