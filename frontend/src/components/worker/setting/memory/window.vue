<template>
  <el-dialog v-model="visible" :title="$t('memory.create-title')" width="528px" :close-on-click-modal="false"
    @opened="handleWindowOpened"
  >
    <el-form :model="loadFormData" label-width="80px" @submit.prevent>
      <el-form-item :label="$t('memory.filename')">
        <el-input ref="loadInputRef" v-model="loadFormData.filename" :placeholder="$t('memory.filename-placeholder')" clearable
